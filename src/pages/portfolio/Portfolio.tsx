...
const Portfolio = () => {
    ...
    const [portfolioSelectedPost, setPortfolioSelectedPost] = useState<any | null>(null);
    ...
    useEffect(() => {
        fetch('data/portfolio/portfolio.xlsx')
            ...
            .then(data => {
                ...
                const validPosts = worksheet.filter((post: any) => post.Title && post.Description && post['Image Link'] && post['Date & Time']);
                const sortedPosts = validPosts.sort((a: any, b: any) => new Date(b['Date & Time']).getTime() - new Date(a['Date & Time']).getTime());
    ...
    const truncatePortfolioText = (text: string, length: number) => {
        const urlRegex = /(https?:\/\/[^\s]+)/g;
        const truncatedText = text.split(urlRegex).filter(part => !urlRegex.test(part)).join('');
    ...
    return (
        <div><Logo />
            <div className='header-portfolio'>
                <h1>
                    My Trash Portfolio 🚮💼🗑️ | Shall we
                    <button className="icon-party-portfolio" id="confettiButton" onClick={handleConfettiClick}>
                        <GiPartyPopper />
                    </button>?
                </h1>
            </div>
            <div className="portfolio-container">
                {portfolioPosts.map((post, index) => (
                    <div key={index} className="portfolio-item" onClick={() => handlePortfolioPostClick(post)}>
                ...
                {portfolioSelectedPost && (
                    <div className="portfolio-modal" onClick={handlePortfolioCloseModal}>
                        <div className="portfolio-modal-content" onClick={(e) => e.stopPropagation()}>
                            <span className="portfolio-close" onClick={handlePortfolioCloseModal}>&times;</span>
                            <div className="portfolio-modal-image-container">
                                <img src={portfolioSelectedPost['Image Link']} alt={portfolioSelectedPost.Title} />
                                <button className="portfolio-fullscreen-icon" onClick={handlePortfolioFullscreenClick}>
                                    <SlSizeFullscreen />
                                </button>
                            </div>
                            <h3>Topic: {portfolioSelectedPost.Title}</h3><strong>📢:</strong> {portfolioSelectedPost['Date & Time']}
                            <p className="portfolio-item-description">{makePortfolioLinksClickable(portfolioSelectedPost.Description)}</p>
                            <p></p>
                        </div>
                    </div>
                )}
                {portfolioIsFullscreen && (
                    <div className="portfolio-fullscreen-modal" onClick={handlePortfolioFullscreenClose}>
